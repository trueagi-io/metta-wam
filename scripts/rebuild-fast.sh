...
(
  ...
  source "./venv8-fast/bin/activate"  # Activate the virtual environment
...
cp -f ~/.cargo/bin/metta ${BUILD_DIR}/venv8-fast/bin/meta-repl 
mv -f ~/.cargo/bin/metta ~/.cargo/bin/metta-repl-fast 
)

(
export SCRIPT=~/.cargo/bin/metta-fast
echo '#!/bin/bash' > $SCRIPT
echo "source ${BUILD_DIR}/venv8-fast/bin/activate" >> $SCRIPT
...
source "$BUILD_DIR/venv8-fast/bin/activate"
