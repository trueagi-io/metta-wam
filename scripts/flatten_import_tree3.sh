...
copy_metta_files all slug my_flat_dir
...
# Step 1: Scan for all .metta files and create mappings
declare -A module_to_file

# Populate module_to_file mapping
while IFS= read -r file; do
    filename=$(basename "$file" .metta)
    module_name="${filename//__/__}"  # Ensures consistency
    module_to_file["$module_name"]="$filename"
done < <(find "$TARGET_DIR" -type f -name "*.metta")

# Step 2: Process each file and correct imports
for file in $(find "$TARGET_DIR" -type f -name "*.metta"); do
    temp_file="${file}.tmp"

    while IFS= read -r line; do
        if [[ "$line" =~ ^\!\(import\!\[[:space:]]*[&]([a-zA-Z0-9_-]+)[[:space:]]+([^)]*)\) ]]; then
            import_prefix="${BASH_REMATCH[1]}"
            import_module="${BASH_REMATCH[2]}"

            # Convert import notation (replace : with __)
            fixed_import="${import_module//:/__}"

            # Lookup the correct module-to-file mapping
