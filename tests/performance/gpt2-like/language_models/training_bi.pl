...
; :- include(weightless_pllm).
  ...
  (= 
    ...
    (, 
      ...
      (set-last-oc (with_self  0 (with_self  0 0))) 
      (absolute-file-name 
        (pldata corpus/self-dialogue-corpus/train-from-topic-star-wars.txt) $File 
        (:: (access read))) 
      (time (, (open $File read $In) (forall (corpus-stat $Stat) (set-flag $Stat 0)) (set-flag sent-num 0) (repeat) (if-then-else (at-end-of-stream $In) (set-det) (, (flag sent-num $X (+ $X 1)) (read-line-to-string $In $Str) (add-training $X $Str) (fail))) (forall (corpus-stat $Stat) (, (get-flag $Stat $Value) (debugln (= $Stat $Value))))))))
    ...
    (, 
      ...
      (is $Y 
        (* 
          (+ $Z10 1) 100)) 
      (flag sent-num $_ $Y) 
      (set-det))); every 10 conversations will be considered "close"
  ...
  (= 
    ...
    (, 
      ...
      (append 
        ...
        (:: (oc $NewOC)) $Grams) 
      (set-det) 
      (add-ngrams except-none ngram 4 $X $Grams)));get_flag(corpus_convos,B),;source_location(_,L),
