...
'mc__1_2_+'(A,B,R) :- type_should_be(number,A),type_should_be(number,B),!,R is A+B.
'mc__1_2_+'(A,B,['+',A,B]).
...
transpiler_predicate_store(builtin, '%', [2], '@doc', '@doc', [x(doeval,eager,[number]), x(doeval,eager,[number])], x(doeval,eager,[number])).
'mc__1_2_%'(A,B,R) :- type_should_be(number,A),type_should_be(number,B),!,R is A mod B.
'mc__1_2_%'(A,B,['%',A,B]).
...
'mc__1_2_<'(A,B,R) :- type_should_be(number,A),type_should_be(number,B),!,(A<B -> R='True' ; R='False').
'mc__1_2_<'(A,B,['<',A,B]).
...
transpiler_predicate_store(builtin, '<=', [2], '@doc', '@doc', [x(doeval,eager,[number]), x(doeval,eager,[number])], x(doeval,eager,[boolean])).
'mc__1_2_<='(A,B,R) :- type_should_be(number,A),type_should_be(number,B),!,(A=<B -> R='True' ; R='False'). % note that Prolog has a different syntax '=<'
'mc__1_2_<='(A,B,['<=',A,B]).
...
'mc__1_2_limit'(N,S,R) :- type_should_be(number,N),N>=0,limit(N,as_p1_exec(S,R)).
