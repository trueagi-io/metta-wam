...
'+'(Addend1, Addend2, Sum):- mnotrace(catch_err(plus(Addend1, Addend2, Sum),_,fail)),!.
...
prolog_only(Goal):- mnotrace(if_t(option_value(prolog,true), call(Goal))).
...
:- abolish(fbug/1).
fbug(Info):- in_cmt(color_g_mesg('#2f2f2f',write_src(Info))).
example0(_):- fail.
