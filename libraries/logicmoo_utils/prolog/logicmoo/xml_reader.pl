...
on_end('xml', _) :- !,
        ignore(retract(in_xml_tag(_))).

on_begin('xml', Attribs, _) :- !,
        asserta(in_xml_tag(Attribs)).


on_begin(Tag, Attr, Parser) :- skipOver(not(inLineNum)),
        get_sgml_parser(Parser,context(Context)), Context=[Tag,xml|_],
        skipOver(debugFmt(on_begin(Tag, Attr, Context))),
        skipOver(retract(in_xml_tag(XmlAttr))),
...
%        load_structure(Stream,Content,[line(Line)|PARSER_DEFAULTS]),!,
 %      skipOver( sgml_parse(Parser,[ document(Content),parse(input)])),
        ...
        skipOver(asserta(in_xml_tag(XmlAttr))),
        assertz(NEW),!.
...
load_xml_structure_lineno(Attributes,Ctx,L):-must_maplist(load_inner_xml_lineno(Attributes,Ctx),L),!.

:-thread_local(t_l:lineInfoElem/4).

load_inner_xml_lineno(Attributes,Ctx,element(Tag,Attribs,ContentIn)):-
   appendAttributes(Ctx,Attributes,Attribs,RightAttribs),
   load_xml_structure(Ctx,element(Tag,RightAttribs,ContentIn)),!.
...
*/
   ...
   load_xml_structure(Ctx,element(Tag,[srcinfo=File0:Line0-Offset0|Attribs],ContentIn)),
...
tls :- string_to_structure('<xml><p>hi</p></xml>',X),debugFmt(X).
...
termFileContents(_Ctx,File,element(xml,[],XMLSTRUCTURES)):- %% another way to fileToLineInfoElements
