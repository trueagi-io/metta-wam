...
process_late_opts:- option_value('html',true), nop(shell('./scripts/total_loonits.sh')).
...
do_metta1(Self,Load,[EQ,Head,Result], Src):- EQ=='=', !,
 must_det_ll((
    ...
    color_g_mesg('#ffa500',
       metta_anew(Load,Src,metta_defn(Self,Head,Result))),
    discover_body(Self,Load,Result))).

do_metta1(Self,Load,PredDecl, Src):-
   ignore(discover_head(Self,Load,PredDecl)),
   color_g_mesg('#ffa500',metta_anew(Load,Src,
     metta_atom(Self,PredDecl))).
...
do_metta_file_exec(Self,Exec):-
  nb_setval(exec_src,Exec),!,
  with_debug(eval,
    with_debug(exec,do_metta_file_exec0(Self,Exec))),
  inc_exec_num,!.
