...
eval_args(A,AA):- eval_args0(A,AA).
eval_args0(A,AA):-
  current_self(Space),
  eval_args0(11,Space,A,AA).

%eval_args0(Depth,_Self,X,_Y):- forall(between(6,Depth,_),write(' ')),writeqln(eval_args0(X)),fail.
eval_args(A,AA,X,Y):- eval_args0(A,AA,X,Y).
eval_args0(_Dpth,_Slf,X,Y):- nonvar(Y),X=Y,!.

eval_args0(Depth,Self,X,Y):- nonvar(Y),!,eval_args0(Depth,Self,X,XX),evals_to(XX,Y).

eval_args0(_Dpth,_Slf,X,Y):- var(X),!,Y=X.

eval_args0(_Dpth,_Slf,[X|T],Y):- T==[], number(X),!,Y=[X].

eval_args0(Depth,Self,[F|X],Y):-
  (F=='superpose' ; ( option_value(no_repeats,false))),
  mnotrace((D1 is Depth-1)),!,
  eval_args00(D1,Self,[F|X],Y).

eval_args0(Depth,Self,X,Y):-
  mnotrace((no_repeats_var(YY),
  D1 is Depth-1)),
  eval_args00(D1,Self,X,Y),
   mnotrace(( \+ (Y\=YY))).
...
eval_args00(Depth,_Slf,X,Y):- Depth<1,!,X=Y, (\+ trace_on_overflow-> true; flag(eval_num,_,0),debug(metta(eval))).
%eval_args00(_Dpth,_Slf,X,Y):- self_eval(X),!,Y=X.
eval_args00(Depth,Self,X,Y):-
  Depth2 is Depth-1,
  eval_args11(Depth,Self,X,M),
  (M\=@=X ->eval_args00(Depth2,Self,M,Y);Y=X).
...
eval_args11(Depth,Self,X,Y):- flag(eval_num,EX,EX+1),
  ...
  (Ret\=@=retval(fail)->true;(rtrace(eval_args00(D1,Self,X,Y)),fail)).
...
eval_args1(Depth,Self,['!',Cond],Res):- !, call(eval_args0(Depth,Self,Cond,Res)).
eval_args1(Depth,Self,['rtrace',Cond],Res):- !, rtrace(eval_args0(Depth,Self,Cond,Res)).
eval_args1(Depth,Self,['time',Cond],Res):- !, time(eval_args0(Depth,Self,Cond,Res)).
eval_args1(Depth,Self,['print',Cond],Res):- !, eval_args0(Depth,Self,Cond,Res),format('~N'),print(Res),format('~N').
% !(println! $1)
eval_args1(Depth,Self,['println!',Cond],Res):- !, eval_args0(Depth,Self,Cond,Res),format('~N'),print(Res),format('~N').

eval_args1(_Dpth,_Slf,List,Y):- is_list(List),maplist(self_eval,List),List=[H|_], \+ atom(H), !,Y=List.

eval_args1(Depth,Self,['assertTrue', X],TF):- !, eval_args0(Depth,Self,['assertEqual',X,'True'],TF).
eval_args1(Depth,Self,['assertFalse',X],TF):- !, eval_args0(Depth,Self,['assertEqual',X,'False'],TF).

eval_args1(Depth,Self,['assertEqual',X,Y],RetVal):- !,
...
eval_args1(Depth,Self,['assertNotEqual',X,Y],RetVal):- !,
   loonit_assert_source_tf(
        ['assertEqual',X,Y],
        (setof_eval(Depth,Self,X,XX), setof_eval(Depth,Self,Y,YY)),
...
eval_args1(Depth,Self,['match',Other,Goal,Template,Else],Template):- into_space(Self,Other,Space),!,  (metta_atom_iter(Depth,Space,Goal)*->true;Else=Template).
...
%[superpose,[1,2,3]]
eval_args1(Depth,Self,['superpose',List],Res):- !, member(E,List),eval_args0(Depth,Self,E,Res).
...
eval_args1(Depth,Self, Term, Res):-
  mnotrace(( get_sa_p1(setarg,ST,Term,P1), % ST\==Term,
   ...
   %max_counting(F,20),
   member(Var,List),
   eval_args0(Depth,Self, Term, Res).
...
eval_args1(Depth,Self,['if',Cond,Then],Res):- !,
   eval_args0(Depth,Self,Cond,TF),
   (is_True(TF) -> eval_args0(Depth,Self,Then,Res) ; Res = []).

eval_args1(Depth,Self,['if',Cond,Then,Else],Res):- !,
   eval_args0(Depth,Self,Cond,TF),
   (is_True(TF) -> eval_args0(Depth,Self,Then,Res);eval_args0(Depth,Self,Else,Res)).

eval_args1(_Dpth,_Slf,[_,Nothing],Nothing):- 'Nothing'==Nothing,!.

eval_args1(Depth,Self,['let',A,A5,AA],OO):- !,
  %(var(A)->true;trace),
  ((eval_args0(Depth,Self,A5,AE), AE=A)),
  eval_args0(Depth,Self,AA,OO).
%eval_args1(Depth,Self,['let',A,A5,AA],AAO):- !,eval_args0(Depth,Self,A5,A),eval_args0(Depth,Self,AA,AAO).
eval_args1(Depth,Self,['let*',[],Body],RetVal):- !, eval_args0(Depth,Self,Body,RetVal).
eval_args1(Depth,Self,['let*',[[Var,Val]|LetRest],Body],RetVal):- !,
    eval_args1(Depth,Self,['let',Var,Val,['let*',LetRest,Body]],RetVal).

eval_args1(Depth,Self,['colapse'|List], Flat):- !, maplist(eval_args0(Depth,Self),List,Res),flatten(Res,Flat).
...
eval_args1(Depth,Self,['Cons', A, B ],['Cons', AA, BB]):- no_cons_reduce, !,
  eval_args0(Depth,Self,A,AA), eval_args0(Depth,Self,B,BB).

eval_args1(Depth,Self,['Cons', A, B ],[AA|BB]):- \+ no_cons_reduce, !,
   eval_args0(Depth,Self,A,AA), eval_args0(Depth,Self,B,BB).


eval_args1(Depth,Self,['change-state!',StateExpr, UpdatedValue], Ret):- !, eval_args0(Depth,Self,StateExpr,StateMonad),
  eval_args0(Depth,Self,UpdatedValue,Value),  'change-state!'(Depth,Self,StateMonad, Value, Ret).
eval_args1(Depth,Self,['new-state',UpdatedValue],StateMonad):- !,
  eval_args0(Depth,Self,UpdatedValue,Value),  'new-state'(Depth,Self,Value,StateMonad).
eval_args1(Depth,Self,['get-state',StateExpr],Value):- !,
  eval_args0(Depth,Self,StateExpr,StateMonad), 'get-state'(StateMonad,Value).



% eval_args1(Depth,Self,['get-state',Expr],Value):- !, eval_args0(Depth,Self,Expr,State), arg(1,State,Value).
...
get_type(Depth,Self,EvalMe,Type):- needs_eval(EvalMe),eval_args0(Depth,Self,EvalMe,Val), \+ needs_eval(Val),!,
   get_type(Depth,Self,Val,Type).
...
eval_args1(Depth,Self,['length',L],Res):- !, eval_args0(Depth,Self,L,LL), !, (is_list(LL)->length(LL,Res);Res=1).
eval_args1(Depth,Self,['CountElement',L],Res):- !, eval_args0(Depth,Self,L,LL), !, (is_list(LL)->length(LL,Res);Res=1).
...
eval_args1(Depth,Self,[F|Args],Res):- is_list(F),
  metta_atom_iter(Depth,Self,['=',F,R]), eval_args0(Depth,Self,[R|Args],Res).

eval_args1(Depth,Self,[F|Args],Res):- is_list(F), Args\==[],
  append(F,Args,FArgs),!,eval_args0(Depth,Self,FArgs,Res).
*/
...
eval_args1(Depth,Self,['bind!',Other,Expr],RetVal):-
   into_name(Self,Other,Name),!,eval_args0(Depth,Self,Expr,Value),nb_setval(Name,Value),  return_empty(Value,RetVal).
eval_args1(Depth,Self,['pragma!',Other,Expr],RetVal):-
   into_name(Self,Other,Name),!,eval_args0(Depth,Self,Expr,Value),set_option_value(Name,Value),  return_empty(Value,RetVal).
eval_args1(_Dpth,Self,['transfer!',File],RetVal):- !, include_metta(Self,File),  return_empty(Self,RetVal).



eval_args1(Depth,Self,['nop',Expr],Empty):- !,  eval_args0(Depth,Self,Expr,_), return_empty([],Empty).
...
eval_args1(Depth,Self,['and',X,Y],TF):- !, as_tf((eval_args0(Depth,Self,X,'True'),eval_args0(Depth,Self,Y,'True')),TF).
eval_args1(Depth,Self,[And,X|Y],TF):- is_and(And),!,eval_args0(Depth,Self,X,TF1),
  is_True(TF1),eval_args1(Depth,Self,[And|Y],TF).
%eval_args2(Depth,Self,[H|T],_):- \+ is_list(T),!,fail.
eval_args1(Depth,Self,['or',X,Y],TF):- !, as_tf((eval_args0(Depth,Self,X,'True');eval_args0(Depth,Self,Y,'True')),TF).
...
eval_args1(Depth,Self,['+',N1,N2],N):- number(N1),!,
   eval_args0(Depth,Self,N2,N2Res), catch(N is N1+N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail)).
eval_args1(Depth,Self,['-',N1,N2],N):- number(N1),!,
   eval_args0(Depth,Self,N2,N2Res), catch(N is N1-N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail)).

eval_args1(Depth,Self,[V|VI],[V|VO]):- nonvar(V),is_metta_data_functor(V),is_list(VI),!,maplist(eval_args0(Depth,Self),VI,VO).
...
%eval_args2_failed(Depth,Self,[V|Nil],[O]):- Nil==[], once(eval_args0(Depth,Self,V,O)),V\=@=O,!.
eval_args2_failed(Depth,Self,[H|T],[HH|TT]):- !,
  eval_args0(Depth,Self,H,HH),
  eval_args2_failed(Depth,Self,T,TT).

eval_args2_failed(Depth,Self,T,TT):- eval_args0(Depth,Self,T,TT).
...
% Kills zero arity functions eval_args1(Depth,Self,[X|Nil],[Y]):- Nil ==[],!,eval_args0(Depth,Self,X,Y).
...
eval_args2(Depth,Self,[F|PredDecl],Res):-
   ...
   eval_args0(Depth,Self,SSub,Repl),
   mnotrace((SSub\=Repl, subst(PredDecl,SSub,Repl,Temp))),
   eval_args0(Depth,Self,[F|Temp],Res).
...
eval_args37(Depth,Self,[H1|Args],Res):-
   ...
   eval_args0(Depth,Self,[H2|H2Args],ArgRes),
...
eval_args37(Depth,Self,[[H|Start]|T1],Y):-
   ...
   eval_args0(Depth,Self,[Left|T1],Y).
...
eval_args40(Depth,Self,[F|X],FY):- is_function(F), \+ is_special_op(F), is_list(X),
  maplist(eval_args0(Depth,Self),X,Y),!,eval_args5(Depth,Self,[F|Y],FY).
...
%eval_args40(Depth,Self,[X1|[F2|X2]],[Y1|Y2]):- is_function(F2),!,eval_args0(Depth,Self,[F2|X2],Y2),eval_args0(Depth,Self,X1,Y1).


cwdl(DL,Goal):- call_with_depth_limit(Goal,DL,R), (R==depth_limit_exceeded->(!,fail);true).
bagof_eval(Depth,Self,X,L):- !,findall(E,eval_args0(Depth,Self,X,E),L).
setof_eval(Depth,Self,X,S):- !,findall(E,eval_args0(Depth,Self,X,E),L),sort(L,S).
%setof_eval(Depth,Self,X,S):- setof(E,eval_args0(Depth,Self,X,E),S)*->true;S=[].
