...
eval_space(Eq,RetType,Depth,Self,['match',Other,Goal,Template],Res):- !,
   eval_match(Eq,RetType,Depth,Self,Other,Goal),
   Template=Res.

eval_match(Eq,_RetType,_Depth,_Slf,Other,[Equal,[F|H],B]):- Eq == Equal,!,  % trace,
...
% is this OK?
eval_match(Eq,RetType,Depth,Self,Other,H):- metta_defn(Eq,Other,H,B), D2 is Depth -1, eval_match(Eq,RetType,D2,Self,Other,B).
