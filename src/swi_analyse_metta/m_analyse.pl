...
% temp(1,<stream>(0x558eef9ae1a0),46,46,
...
try_clean_python_to_metta( Lx, Lx3 ):- sub_string( Lx, _,_,_, "self.assertEqualMettaRunnerResults" ),
 ...
 today_key( Dk ), 
 incr2( N2 ), number_string( N2, N2_s ),
 concat_slist( [ "\n;<br>", N2_s, " ", Dk, " ", "R.v.V converted python source to metta !(assertEqualToResult ( )  (  )  ) ", "\n;<br>\n" ], "", New_s ),
...
trigger_code_open_close( Lx ):- 
  ...
  sub_string( Lx, _, _, _, "<button title=\"Copy Code" ), !,
  %sub_string( Lx, _, _, _, "span class=\"line\"" ), 
  %sub_string( Lx, _, _, _, "(</span>" ),   !,
  incr( N2 ),  
  retractall( is_open_code( _, _  ) ), assert( is_open_code( 1 , N2 ) ).
  %fail .



%trigger_code_open_close( Lx ):- 
%  sub_string( Lx, _, _, _, "code-runner-control" ), !,
% retractall( is_open_code( _ ) ), assert( is_open_code( 0 ) ).

trigger_code_open_close( Lx ):- 
  sub_string( Lx, _, _, _, "id=\"play-icon\"" ), !,
  retractall( is_open_code( _ , _ ) ), assert( is_open_code( 0 , 0 ) ).

trigger_code_open_close( _Lx ):-  !.


%found_search_result(metta_lang_dev,"",0,9786,'metta_lang_dev/Handling nondeterministic results _ MeTTa Language.html',
% "<span class=\"line\"><span style=\"--shiki-light: #24292E; --shiki-dark: #E1E4E8;\">! </span><span style=\"--shiki-light: #22863A; --shiki-dark: 
% #85E89D;\">(</span><span style=\"--shiki-light: #6F42C1; --shiki-dark: #B392F0;\">collapse</span><span style=\"--shiki-light: #22863A; 
% --shiki-dark: #85E89D;\"> (</span><span style=\"--shiki-light: #24292E; --shiki-dark: #E1E4E8;\">color</span><span style=\"--shiki-light: 
% #22863A; --shiki-dark: #85E89D;\">))</span><span style=\"--shiki-light: #6A737D; --shiki-dark: #6A737D;\"> ; [(green yellow red)]</span></span>")

%remove_angle_tags( Lx2, Lx3 ):-
% sub_str_between_plus_rest( Lx2, "<", ">", _Subz , Subz2 ), !,
% remove_angle_tags( Subz2, Lx3 ).
%remove_angle_tags( Lx2, Lx2 ):- !.

%try_get_code_string( Lx, Lx2 ):-    sub_str_between( Lx, "! </sp", ")</sp", Lx2 ), !.
try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #24292E; --shiki-dark: #E1E4E8;\">", "</span>", Tag , Rest , Pos ).

try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #6A737D; --shiki-dark: #6A737D;\">", "</span>", Tag , Rest , Pos ).

try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #22863A; --shiki-dark: #85E89D;\">", "</span>", Tag , Rest , Pos ).
 
try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #6F42C1; --shiki-dark: #B392F0;\">", "</span>", Tag , Rest , Pos ).


try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #E36209; --shiki-dark: #FFAB70;\">", "</span>", Tag , Rest , Pos ).

try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #D73A49; --shiki-dark: #F97583;\">", "</span>", Tag , Rest , Pos ).

try_get_tag_nd( Lx, Tag, Rest , Pos ):-
 sub_str_between_plus_rest_position( Lx, "<span style=\"--shiki-light: #005CC5; --shiki-dark: #79B8FF;\">", "</span>", Tag , Rest , Pos ). 
 
 % #E36209; --shiki-dark: #FFAB70
   
   
%   <span style="--shiki-light: #24292E; --shiki-dark: #E1E4E8;">! </span>
%   <span style="--shiki-light: #22863A; --shiki-dark: #85E89D;">(</span>
%   <span style="--shiki-light: #6F42C1; --shiki-dark: #B392F0;">get-type</span>
%   <span style="--shiki-light: #22863A; --shiki-dark: #85E89D;"> (</span>
%   <span style="--shiki-light: #24292E; --shiki-dark: #E1E4E8;">a b</span>
%   <span style="--shiki-light: #22863A; --shiki-dark: #85E89D;">))</span>
%   <span style="--shiki-light: #6A737D; --shiki-dark: #6A737D;"> ; (A B)</span>
% --shiki-light: #22863A; --shiki-dark: #85E89D;

%found_search_result(metta_lang_dev,"",0,9733,'metta_lang_dev/Concrete types _ MeTTa Language.html',"(: b B)")
%found_search_result(metta_lang_dev,"",0,9741,'metta_lang_dev/Concrete types _ MeTTa Language.html',"(: a A)")
%found_search_result(metta_lang_dev,"",0,9742,'metta_lang_dev/Concrete types _ MeTTa Language.html',"(: foo (-&gt; A B))")
%ound_search_result(metta_lang_dev,"",0,9745,'metta_lang_dev/Concrete types _ MeTTa Language.html',"( ($ infix-f $) ($ $))")

try_get_tag_closest( Lx, Tag, Rest ):-
  findall( Pos, try_get_tag_nd( Lx, _, _ , Pos ), PosL ),
  sort( PosL, PosL2 ),
  member( Posx, PosL2 ),
  try_get_tag_nd( Lx, Tag, Rest , Posz ),  Posx == Posz ,!.


try_get_code_string( Lx, Hs, Lx2 ):-    
  try_get_tag_closest( Lx, Tag, Rest ),
  % findall( try_get_tag_nd( Lx, Tag, Rest , Pos ), !,
  string_concat( Hs, Tag, Hs2 ),
  try_get_code_string( Rest, Hs2, Lx2 ).
try_get_code_string( _Lx, Hs, Hs ):-     !.

 % sub_str_between( Lx, "<span class=\"line\">", "</span></span>", Lx2 ), !.

wri_lang_dev_code( Lx , Linum , F ):- 
   is_open_code( Is_open , Count ), Is_open == 1,
   
%   assert( found_search_result( 'metta_lang_dev', "" , 0, Linum, F, Lx  ) ), 
   
   try_get_code_string( Lx, "", Lx2 ),
 %  remove_angle_tags( Lx2, Lx2 ),
   string_length( Lx2, Lex ), Lex > 0,
   %write( Lx2 ), nl, 
%   sub_str_between( Lx, Tag_from, Tag_til, Subz10 ),
%   replace
%   str_code_remove( Subz10, 92,  Subz1 ),
...
%   sub_str_between( Lx, Second_tag_begin, Second_tag_end, Subz2 ), 
       file_base_name( F , Fn ),
   %assert_eval_tag( 'metta_lang_dev', Linum, "", F, Lx  , 0),
%   not( eval_tag( _, Subz1, _ ) ),  assert( eval_tag( Linum, Subz1, F ) ),  
   
   assert( found_search_result( 'metta_lang_dev', "" , Count, Linum, Fn, Lx2  ) ), 
   % write( Lx ), nl,
...
%wri_rust( Lx , _, F ):- 
% sub_string( Lx , _, _, _ , "regex" ), !, write("is_line-"), write( Lx ), nl .
wri_lang_dev_code( _Lx, _ , _ ):- !.


read_metta_lang_dev_stream( Sea  , N, Fnx ):-
 read_line_to_string( Sea, Lx ), Lx \= end_of_file, 
 % write( Lx ), nl,
 trigger_code_open_close( Lx ), !,
 wri_lang_dev_code( Lx , N , Fnx ),  
 N2 is N + 1,
 read_metta_lang_dev_stream( Sea , N2 , Fnx).
...
read_metta_lang_dev_stream( _Sea  , _ ,_):- !.
...
%--
read_is_ready_test_sea( Sea  , N, Token, Fnx, 1 ):-
 read_line_to_string( Sea, Lx ), Lx \= end_of_file, 
 % write( Lx ), nl,
 sub_string( Lx, _, _, _, Token ),  !.
 %write( Lx ), nl, 
 %N2 is N + 1,
 %read_is_ready_test_sea( Sea , N2 , Fnx).
read_is_ready_test_sea( Sea  , N, Token, Fnx, Res ):-    not( at_end_of_stream( Sea ) ), !, read_is_ready_test_sea( Sea , N , Token, Fnx, Res ).
read_is_ready_test_sea( _Sea  , _ ,_,_, 0 ):- !.


%----
read_is_ready_test( Cx , Token, Res ):-
 open( Cx , read, Sea , [ encoding(octet) ] ), 
 read_is_ready_test_sea( Sea, 1, Token, Cx , Res ),  
 close( Sea ).

 

check_ready_tests():-
 % Dir = '../../tests/from_python_generated/',
 Dir = '../../tests/metta_lang_dev/',
 directory_files(Dir, Lis), member(El, Lis), El \= '..', El \= '.',
 atom_concat(Dir, El, Cx), 
%  write( Cx ), nl, 
  %Token = "ready works",
  Token = "notready",
 read_is_ready_test( Cx , Token, Res ),  Res == 1,
 write( Cx ), write( " " ), write( Token ), nl, 
 fail, !.
check_ready_tests():- !.
...
convert_metta_lang_dev( Fnx ):-
 ...
 open( Fnx , read, Sea , [ encoding(utf8) ] ), 
 read_metta_lang_dev_stream( Sea, 1, Fnx ),  
 close( Sea ).
