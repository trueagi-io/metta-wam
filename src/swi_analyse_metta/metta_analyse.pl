...
% , 
read_extra_lines_max_until_full_metta_claus( Count_is_open, Count, _Max, _Sea , StrBuf, StrBuf, Count ):-  Count_is_open == 0, !.


read_extra_lines_max_until_full_metta_claus( Count_is_open, Count, Max, Sea , StrBuf, Xtra_lines, ResultCount ):-   Count < Max,
	...
	Count2 is Count + 1,
  read_extra_lines_max_until_full_metta_claus( NewCount, Count2, Max, Sea , StrBuf2, Xtra_lines, ResultCount ).
  

read_extra_lines_max_until_full_metta_claus( _Count_is_open, Count, _Max, _Sea , StrBuf, StrBuf, Count ):-   !.
%--
...
search_o_y_and_assert( 1, _Sea, Linum, Lx, _Zk, F, Level, Linum , is_not_leading, 0 ):-
...
search_o_y_and_assert( 1, _Sea, Linum, Lx, _Zk, F, Level, Linum , _Isleading,  0 ):-
...
search_o_y_and_assert( 1, Sea, Linum, Lx, _Zk, F, Level, Linum , metta_else_where , Lines_read_extra ):-
...
search_o_y_and_assert( 1, Sea, Linum, Lx, _Zk, F, _Level, Linum , metta_std_lib , ResultCount ):-
  not_exclude_metta_file( F ),  file_name_extension( _, Ext, F ), Ext == 'metta',
  eval_pl_tag_or_rs_tag( Tag_s ),    metta_docfile_tag( Tag_from, Tag_til ),
  string_concat( Tag_from, Tag_s, C1 ),   string_concat( C1, Tag_til, C4 ), 
  search_o_y( Lx, C4 ), !,  parenthesis_count( Lx, Count_is_open ), 
  read_extra_lines_max_until_full_metta_claus( Count_is_open, 0, 8, Sea , "", Xtra_lines, ResultCount ), 
...
search_o_y_and_assert( 1, Sea, Linum, Lx, _Zk, F, Level, Linum ,  is_not_leading , Lines_read_extra):-
  not_exclude_metta_file( F ),  file_name_extension( _, Ext, F ), Ext == 'metta',
  eval_pl_tag_or_rs_tag( Tag_s ),    metta_docfile_tag( Tag_from, Tag_til ),
...
search_o_y_and_assert( 1, Sea, Linum, Lx, _Zk, F, Level, Linum , is_not_leading, ResultCount ):-
  not_exclude_metta_file( F ),   file_name_extension( _, Ext, F ), Ext == 'metta',
...
% kopie 
search_o_y_and_assert( 1, _Sea, Lx, Linum, _Zk, F, Level, Linum , is_leading, 0 ):-
  sub_atom( F, _, _, _, '.py' ),  
  python_detect_tag( Tag_from, Tag_til ),
...
read_has_search_stream( Sea, Zk, F, Level, Linum, Is_include, Isleading):- not( at_end_of_stream( Sea ) ),
 read_line_to_string( Sea, Lx ), Lx \= end_of_file, 
   ...
   % if_extra_string_demand_search( Lx ),
   search_o_y_and_assert( Is_include, Sea, Linum, Lx, Zk, F, Level, Linum , Isleading, Result_lines_red ),
