...
transpiler_predicate_nary_store(builtin, 'py-atom-call', 1, ['Expression'], 'Atom', 'Atom', [x(noeval,eager,[])], x(doeval,eager,[]), x(doeval,eager,[])).
'mc_n_1__py-atom-call'(SymRef,Args,Ret) :- 'mc_n_1__py-atom-call!'(SymRef,Args,Ret).

transpiler_predicate_nary_store(builtin, 'py-atom-call!', 1, ['Expression'], 'Atom', 'Atom', [x(noeval,eager,[])], x(noeval,eager,[]), x(doeval,eager,[])).
'mc_n_1__py-atom-call!'(SymRef,Args,Res) :-
    py_call_method_and_args_sig(_RetType,[],SymRef,Args,Res).

:- initialization(setup_library_call(builtin, 'eval-string', [1], ['String'], 'Atom', [x(doeval,eager,[])], x(noeval,eager,[])),program).
'mc__1_1_eval-string'(String,Res) :-
    eval_string(String,Res).


:- initialization(setup_library_call(builtin,'eval-in-only', [2], ['String'], 'Atom', [x(doeval,eager,[]), x(noeval,eager,[])], x(doeval,eager,[])),program).
'mc__1_1_eval-string'(String,Res) :-
    eval_string(String,Res).
...
transpiler_predicate_nary_store(builtin, 'py-atom', 1, ['Atom'], 'Atom', 'Atom', [x(noeval,eager,[])], x(noeval,eager,[]), x(doeval,eager,[])).
'mc_n_1__py-atom'(SymRef,Specialize,ResO) :-
   py_atom(SymRef,Res), specialize_res(Res,Specialize,ResO).

transpiler_predicate_nary_store(builtin, 'py-dot', 2, ['Atom','Atom'], 'Atom', 'Atom', [x(noeval,eager,[]),x(noeval,eager,[])], x(noeval,eager,[]), x(doeval,eager,[])).
'mc_n_2__py-dot'(Arg1,Arg2,Specialize,ResO) :-
   make_py_dot(Arg1,Arg2,Res),specialize_res(Res,Specialize,ResO).


transpiler_predicate_nary_store(builtin, 'py-dot-call', 1, ['Expression'], 'Atom', 'Atom', [x(noeval,eager,[])], x(doeval,eager,[]), x(doeval,eager,[])).
'mc_n_1__py-dot-call'(SymRef,Args,Ret) :- 'mc_n_1__py-dot-call!'(SymRef,Args,Ret).

transpiler_predicate_nary_store(builtin, 'py-dot-call!', 1, ['Expression'], 'Atom', 'Atom', [x(noeval,eager,[])], x(noeval,eager,[]), x(doeval,eager,[])).
'mc_n_1__py-dot-call!'(SymRef,Args,Ret) :- eval_in_only(interp,[['py-dot'|SymRef]|Args],Ret).
